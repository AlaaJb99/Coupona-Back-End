...
public class CouponService {
	...
	public void isPurchased(long customerId, int couponId) {
		PurchaseCoupon puCoupon = purchaseCouponDAO.findByCustomerIdAndCouponId(customerId, couponId);
		// check if this customer already purchased this coupon
		if (puCoupon != null) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Already purchased");
	...
	public void deleteExpiredCoupons() {
		...
		List<Coupon> deleteCoupons = couponDAO.findByEndDate(today);
	...
	private double[][] userSimilarity(int[][] interMatrix, int n) {
		...
		log.info("usersimilarity");
		return userSim;
	}

	private int[] findKNearestCustomers(double[] similar, int customerIndex, int k) {
		int numberofNotSimilar = 0;
		double[] similarCusto = Arrays.copyOf(similar, similar.length);
		log.info("find knearst");
		int iPlace=0;
		List<Integer> indexes = new ArrayList<>();
		
		log.info(similar.length+" the number of similar");
		log.info(similar.toString()+" the of similar");
		for (int i = 0; i < k; i++) {
			double maxi = Double.MIN_VALUE;
			for (int j = 0; j < similar.length; j++) {
				if (maxi < similarCusto[j] && similarCusto[j] > 0 && customerIndex!=j) {
					log.info(similarCusto[j] + " " + j);
					maxi = similarCusto[j];
					indexes.add(i, j);
					iPlace = j;
				}
				else if (similarCusto[j] == 0) {
					numberofNotSimilar++;
				}
				similarCusto[iPlace] = Double.MIN_VALUE;
			}
		}
		if(indexes.size()>0) {
		...
		for (int i = 0; i < customersIndex.length; i++) {
			...
			for (int j = 0; j < customer.length; j++) {
				if (customer[j] == 1 && thisCust[j] == 0) {
					...
					log.info(couponIndex+" couponindex");
	...
	public List<Coupon> knnRecommendation(long c) {
		...
		if (purchased.size() == 0)
			...
			int[] kindex = findKNearestCustomers(similarCustomers, customerId.indexOf(c), k);
			
			List<Coupon> recommCoupons = new ArrayList<>();
			if (kindex != null)
				// find the coupons that these customers bought and this customer didn't
				recommCoupons = getRecommendedCoupons(kindex, matrix, couponsId, customerId.indexOf(c));
